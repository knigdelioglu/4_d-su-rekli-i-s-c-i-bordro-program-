...
#[tauri::command]
pub fn get_institution_settings(db: State<'_, DbState>) -> Result<HashMap<String, DonemselKurumDegerleri>> {
    let conn = db.lock().unwrap();
    SettingsRepository::get_all_institution_settings(&conn)
}

#[tauri::command]
pub fn save_institution_settings(db: State<'_, DbState>, settings: DonemselKurumDegerleri) -> Result<()> {
    let conn = db.lock().unwrap();
