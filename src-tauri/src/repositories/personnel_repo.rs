...
impl PersonnelRepository {
    pub fn get_all(conn: &Connection) -> Result<Vec<Personel>> {
        ...
        let rows = stmt.query_map([], |row| {
            let sendika_uyesi: i32 = row.get(10)?;
            let sabit_sendika: Option<i64> = row.get(11)?;
            let bes_uyesi: i32 = row.get(12)?;
            let oks_orani: Option<i64> = row.get(13)?;
            let sabit_bes: Option<i64> = row.get(14)?;
            let icra: Option<i64> = row.get(15)?;
            let kisi_borcu: Option<i64> = row.get(16)?;
            let dogum: Option<i64> = row.get(17)?;
            let hayat: Option<i64> = row.get(18)?;
            let diger: Option<i64> = row.get(19)?;

            let kesintiler = PersonelKesintileri {
    ...
    pub fn save(conn: &Connection, p: &Personel) -> Result<()> {
        ...
        let sendika_uyesi = if k.and_then(|k| k.sendikaUyesi).unwrap_or(false) { 1 } else { 0 };
        let bes_uyesi = if k.and_then(|k| k.besUyesi).unwrap_or(false) { 1 } else { 0 };
