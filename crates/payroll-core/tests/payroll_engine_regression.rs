...
use payroll_core::{
    ...
    CanonicalTaxCalculationInput, DevredenPekKaydi, DonemselKurumDegerleri, GelirKalemleri,
    ManualPayrollIncomeInput, MealExemptionSegment, PayrollAccrualInput, PayrollCalculationRequest,
    PayrollDatasetSnapshot, PayrollMutation, Personel, PersonelPuantaj, PuantajOzeti,
    StatutoryParameterSegment, StatutorySnapshotSource,
};
...
fn standalone_supplementary_request(payment_date: &str) -> PayrollCalculationRequest {
    let mut request = supplementary_request(payment_date);
...
fn canonical_tax_calculator_matches_the_payroll_engine_final_snapshot() {
    ...
    let canonical = calculate_canonical_tax_state(&CanonicalTaxCalculationInput {
        ...
        previous_cumulative_gv: result.gvDetay.as_ref().unwrap().oncekiKumulatifGvMatrahi,
    ...
    assert_eq!(
        canonical.gv_base,
        result.gvDetay.as_ref().unwrap().cariGvMatrahi
    );
    assert_eq!(
...
fn supplementary_is_independent_and_only_prior_events_must_be_authoritative() {
    ...
    earlier
        .dataset
        .payrolls
        .push(calculate_payroll(&explicit_normal_request("2026-02-14")).unwrap());
...
fn supplementary_events_calculate_without_attendance_but_normal_still_fails_closed() {
    for (sequence, accrual_type) in [
        ...
        let payroll =
            calculate_payroll(&request).expect("supplementary event is attendance-independent");
...
fn provisional_supplementary_finalization_is_rejected_but_complete_attendance_is_authoritative() {
    let mut provisional_request = standalone_supplementary_request("2026-01-31");
    ...
    let authoritative = calculate_payroll(&standalone_supplementary_request("2026-01-31"))
...
fn incomplete_supplementary_attendance_remains_provisional() {
    ...
    let payroll = calculate_payroll(&request)
        .expect("incomplete attendance must not block supplementary calculation");
    assert_eq!(
        payroll.statutorySnapshot.as_ref().unwrap().source,
        StatutorySnapshotSource::ProvisionalPaymentMonth
    );
...
#[test]
fn attendance_mutation_invalidates_attendance_dependent_supplementary_events() {
    let mut provisional_request = supplementary_request("2026-02-10");
    provisional_request.dataset.attendances.clear();
    let provisional =
        calculate_payroll(&provisional_request).expect("provisional event should calculate");
    ...
    .expect("attendance mutation policy should calculate");
    assert_eq!(authoritative_impact.affectedPayrolls.len(), 1);
    assert!(authoritative_impact.blockedByFinalized.is_empty());
}
...
fn provisional_supplementary_recalculates_as_attendance_backed_then_finalizes() {
    ...
    let mut recalculation_request = standalone_supplementary_request("2026-01-31");
...
#[test]
fn legacy_finalized_provisional_supplementary_state_returns_blocker() {
    let mut request = supplementary_request("2026-02-10");
    request.dataset.attendances.clear();
    ...
    .expect("policy should return the relevant finalized blocker");
...
fn normal_after_attendance_free_tediye_reuses_payment_event_state() {
    ...
    let tediye =
        calculate_payroll(&tediye_request).expect("attendance-free tediye should calculate");
...
fn early_provisional_tediye_reconciles_before_low_day_normal() {
    ...
    let tediye =
        calculate_payroll(&tediye_request).expect("early provisional tediye should calculate");
...
fn early_over_cap_supplementary_carry_is_reconciled_without_deadlock() {
    ...
    let tediye =
        calculate_payroll(&tediye_request).expect("over-cap provisional tediye should calculate");
...
fn same_month_event_order_reconciles_to_the_same_final_pek_state() {
    ...
    let normal_first =
        calculate_payroll(&normal_first_request).expect("NORMAL-first event should calculate");
    ...
    tediye_first_request.accrual.as_mut().unwrap().grossAmount = Some(dec!(200000));
...
fn payment_event_chain_shares_snapshots_and_preserves_full_normal_income() {
    for same_day in [false, true] {
        ...
        let mut normal_request =
            explicit_normal_request(if same_day { "2026-02-10" } else { "2026-02-14" });
        ...
        assert_eq!(
            normal.gelirler.tabanBrutAylik,
            standalone.gelirler.tabanBrutAylik
        );
        ...
        assert!(
            first.pekDetay.as_ref().unwrap().primMatrahi
                + normal.pekDetay.as_ref().unwrap().primMatrahi
                <= normal.statutorySnapshot.as_ref().unwrap().pekUstSinir
        );
        assert_eq!(
            serde_json::to_value(&normal.devredenPekGelen).unwrap(),
            serde_json::to_value(&first.sonrakiDevredenPek).unwrap()
        );
        ...
        third_request.accrual = Some(PayrollAccrualInput {
            accrualId: "third-tis".into(),
            accrualType: AccrualType::TIS_IKRAMIYE,
            paymentDate: if same_day { "2026-02-10" } else { "2026-02-25" }.into(),
            sequence: if same_day { 2 } else { 0 },
            grossAmount: Some(dec!(2000)),
            description: None,
        });
        let third = calculate_payroll(&third_request).unwrap();
        assert_eq!(
            third.gvDetay.as_ref().unwrap().oncekiKumulatifGvMatrahi,
            first_gv.cariGvMatrahi + gv.cariGvMatrahi
        );
        assert_eq!(
            third.gvDetay.as_ref().unwrap().uygulananGvIstisnasi,
            dec!(0)
        );
        assert_eq!(
            serde_json::to_value(&third.devredenPekGelen).unwrap(),
            serde_json::to_value(&normal.sonrakiDevredenPek).unwrap()
        );
        ...
        assert!(calculate_payroll(&third_request)
            .unwrap_err()
            .to_string()
            .contains("yalnız bir NORMAL"));
    }
}
...
fn first_supplementary_event_advances_carry_month_once() {
    ...
    assert_eq!(
        previous.sonrakiDevredenPek.as_ref().unwrap()[0].kalanAySayisi,
        2
    );
