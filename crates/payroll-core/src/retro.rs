...
mod tests {
    ...
    fn attendance_summary_counts_each_rest_and_leave_code_independently() {
        let mut summary = PuantajOzeti::default();
        for code in ["G", "İ", "GÇT", "G", "İ", "GÇT"] {
            add_summary(&mut summary, code, "source-period").expect("valid attendance code");
        }
        assert_eq!(summary.g, 2);
        assert_eq!(summary.i, 2);
        assert_eq!(summary.gct, 2);
        assert_eq!(summary.c, 0);
        assert_eq!(summary.t, 0);
        assert_eq!(summary.gc, 0);
        assert_eq!(summary.r, 0);
        assert!(add_summary(&mut summary, "UNKNOWN", "source-period").is_err());
        assert_eq!(summary.g, 2);
        assert_eq!(summary.i, 2);
        assert_eq!(summary.gct, 2);
    }
    ...
    #[test]
    fn signed_distribution_uses_only_matching_signs_and_conserves_exact_total() {
        let desired = [dec!(10), dec!(20), dec!(-10), dec!(-20), Decimal::ZERO];
        let positive = distribute_signed_delta(dec!(9), &desired);
        assert_eq!(
            positive,
            vec![dec!(3), dec!(6), dec!(0), dec!(0), dec!(0)]
        );
        assert_eq!(positive.iter().copied().sum::<Decimal>(), dec!(9));

        let negative = distribute_signed_delta(dec!(-9), &desired);
        assert_eq!(
            negative,
            vec![dec!(0), dec!(0), dec!(-3), dec!(-6), dec!(0)]
        );
        assert_eq!(negative.iter().copied().sum::<Decimal>(), dec!(-9));
        assert_eq!(
            distribute_signed_delta(Decimal::ZERO, &desired),
            vec![Decimal::ZERO; desired.len()]
        );
    }

    #[test]
    fn signed_distribution_assigns_rounding_residual_to_final_eligible_code() {
        assert_eq!(
            distribute_signed_delta(dec!(0.01), &[dec!(1), dec!(1), dec!(1)]),
            vec![dec!(0), dec!(0), dec!(0.01)]
        );
        assert_eq!(
            distribute_signed_delta(dec!(-0.01), &[dec!(-1), dec!(-1), dec!(-1)]),
            vec![dec!(0), dec!(0), dec!(-0.01)]
        ...
        assert_eq!(
            distribute_signed_delta(dec!(7), &[Decimal::ZERO, Decimal::ZERO]),
